 ...
 *	    10 - apr - 2007 (J. R. Valverde) Added a function to destroy a list
 *								
 *	COPYRIGHT						
...
/*--------------------------------------------------------------------
	LIST_DESTROY
		Destroy a list and all its contents, returning the memory occupied
	to the system
	INPUT:	LIST -- The list to destroy
		DESTROY_ITEM -- a function that can free the space used by items

	J. R. VALVERDE
----------------------------------------------------------------------*/
	

list_destroy(list, destroy_item)
list_t list;
void (*destroy_item)(info_t *);
{
	l_node_t *cur, *next;
