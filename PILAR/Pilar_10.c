...
*								*
*	SYNOPSIS						*
*	    PILAR_10 -- tenth PILAR program. Counts the electric*
...
*	PILAR_10 -p protein -o output -g -n -t			*
*								*
...
*	of + - 1.						*
*	    To count interactions it considers +/- 3 residues	*
*	to each side of the considered one.			*
...
char *argv[];
{
	...
	printf("PILAR_10: Count interactions between two sequences.\n");
...
protein_t *prot;
{
    ...
    len = prot->p_seqlen;
    /* Add 6 padd values for +/-3 computation */
    charges = (float *) calloc(len + 6, sizeof(float));
    prot->p_aaval[0] = charges;

    /* assign charges */
    charges[0] = charges[1] = charges[2] = 0;
    for (i = 0; i < len; ++i)
	switch (prot->p_seq[i]) {
	    case 'K':
	    case 'k':
    ...
    charges[len] = charges[len + 1] = charges[len + 2] = 0;
}
