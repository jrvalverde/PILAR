 ...
 *  a protein from a EMBL-format protein database.
 *
 *  Synopsis:
 *	This module provides the function
 *	    status EMBL_next_protein(db, prot)
 *	    db_t *db;
 *	    protein_t *prot;
 *
...
protein_t *prot;
{
    ...
    /* db_buffer contains the first line in the entry:
    "ID345Seq1	ID... " */
    memset(prot->p_name, '\0', PNAME_LEN);
    strncpy(prot->p_name, &db->db_buffer[6], PNAME_LEN - 1);

    /* skip comment lines, until we find the ORIGIN line
    which opens the sequence */
    while ((strindex(db->db_buffer, "SQ   ") != 0)
	|| (is_empty(db->db_buffer))) {
	...
	break;		/* end of sequence, release '88 */
      if (strindex(db->db_buffer, "ID   ") == 0)
	break;		/* but former releases hadn't it */
 ...
 */
  {
    while (strindex(db->db_buffer, "ID   ") != 0) {
      db->db_pos = ftell(db->db_fp);
