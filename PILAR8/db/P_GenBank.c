 ...
 *  a protein from a GenBank-format protein database.
 *
 *  Synopsis:
 *	This module provides the function
 *	    status GenBank_next_protein(db, prot)
 *	    db_t *db;
 *	    protein_t *prot;
 *
...
protein_t *prot;
{
    ...
    while ((strindex(db->db_buffer, "ORIGIN") != 0)
 ...
 */
  {
    while (strindex(db->db_buffer, "LOCUS") != 0) {
      db->db_pos = ftell(db->db_fp);
