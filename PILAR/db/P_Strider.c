 ...
 *  a protein from a DNA Strider-format protein database.
 *
 *  Synopsis:
 *	This module provides the function
 *	    status Strider_next_protein(db, prot)
 *	    db_t *db;
 *	    protein_t *prot;
 *
...
protein_t *prot;
{
    ...
    /* db_buff contains now first line:
    ; ### from DNA Strider  ...	*/
    ...
    while ((db->db_buffer[0] == ';') || (is_empty(db->db_buffer))) {
	db->db_pos = ftell(db->db_fp);
	if (fgets(db->db_buffer, MAXLINE, db->db_fp) == NULL)
	    return FALSE;	/* EOF */
    }
